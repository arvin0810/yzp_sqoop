...
public class EvalSqlTool extends BaseSqoopTool {
  ...
  public int run(SqoopOptions options) {
    ...
    try {
      ...
      //yangzhenpeng add
      // replace $CONDITIONS
      query = query.replaceAll("\\$CONDITIONS", "(1 = 1)");
      Boolean formatted = options.getFormatted();
      Boolean calcrows = options.getCalcRows();
      ...
      while (true) {
        ...
        if (!resultType) {
          ...
          if (calcrows) {
            new ResultSetPrinter().printResultNum(pw, rs);
          } else if (formatted) {
            new ResultSetPrinter().printResultSet(pw, rs, formatted);
          } else {
