...
public class ResultSetPrinter {
  ...
  public final void printResultSet(PrintWriter pw, ResultSet results)
    ...
    try {
      ...
      for (int i = 1; i < cols + 1; i++) {
        ...
        sbNames.append(colName == null ?  "(null)":colName);
        if (i < cols) sbNames.append(",");
      }
      sbNames.append('\n');
      ...
      while (results.next())  {
        ...
        for (int i = 1; i < cols + 1; i++) {
          sb.append(results.getString(i) == null ? "(null)":results.getString(i));
          if (i < cols) sb.append(",");
        }
        sb.append('\n');
  ...
  //yangzhenpeng@huizhongcf.com add:
  // original code is printResultSet(pw, rs), and the output is formatted, let's reverse the logic.
  //output the result set formatted if the parameter is added, not if the parameter is not explicit specified.
  public final void printResultSet(PrintWriter pw, ResultSet results, boolean formatted)
          throws IOException {
    try {
      StringBuilder sbNames = new StringBuilder();
      int cols = results.getMetaData().getColumnCount();
  ...
  //yangzhenpeng@huizhongcf.com add:
  // this function simply output the number of rows which result set has returned.
  public final void printResultNum(PrintWriter pw, ResultSet results)
          throws IOException {
    try {
      int rows = 0;
