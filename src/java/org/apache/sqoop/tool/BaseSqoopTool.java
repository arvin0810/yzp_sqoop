...
public abstract class BaseSqoopTool extends org.apache.sqoop.tool.SqoopTool {
  ...
  // 20190823 yangzhenpeng add, for eval tool to output result without formatted
  ...
  protected RelatedOptions getCommonOptions() {
    ...
    //yangzhenpeng add
    commonOpts.addOption(OptionBuilder
            .withDescription("Print resultset with table formatted")
            .withLongOpt(FORMATTED_ARG)
            .create());
    commonOpts.addOption(OptionBuilder
            .withDescription("Print the number of rows which query has returned")
            .withLongOpt(CALCROWS_ARG)
            .create());
  ...
  protected void applyCommonOptions(CommandLine in, SqoopOptions out)
    ...
    if (in.hasOption(FORMATTED_ARG)) {
      // Immediately switch into DEBUG logging.
      out.setFormatted(true);
      LOG.debug("Enabled formatted output.");
    }

    if (in.hasOption(CALCROWS_ARG)) {
      out.setCalcRows(true);
      LOG.debug("Enabled row count calculating.");
    }
