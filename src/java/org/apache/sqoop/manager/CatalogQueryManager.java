...
public abstract class CatalogQueryManager
  ...
  protected abstract String getListColumnCommentsQuery(String tableName);
  @Override
...
//      LOG.debug("Using query: " + listColumnsQuery + " to look up column names.");
//      LOG.debug(columns.toString());
  ...
  public List<Map<String, String>> getColumnComments(String tableName) {
    ...
    String listColumnCommentsQuery = getListColumnCommentsQuery(tableName);
    try {
      c = getConnection();
      s = c.createStatement();
      rs = s.executeQuery(listColumnCommentsQuery);
      LOG.debug("Using query: " + listColumnCommentsQuery + " to look up column's comment.");
      while (rs.next()) {
