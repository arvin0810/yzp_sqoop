...
public class TableDefWriter {
  ...
  private String [] getColumnNames() {
    ...
    LOG.debug("input table name and output table name : " + inputTableName + ":" + outputTableName);
    LOG.debug("Using manager: " + connManager.getClass().toString());
    if (null != colNames) {
  ...
  private List<Map<String, String>> getColumnComments() {
    if (null != inputTableName) {
      return connManager.getColumnComments(inputTableName);
    } else {
        String pattern = "\\s*(select)\\s*(.+?)\\s*(from)\\s*(.+?)[ ;]\\s*.*";
        String query = options.getSqlQuery().toLowerCase();
        // yangzhenpeng add
        // will find the <database>.<table> info from the query, thus we get the table's name
        // we assume that the query string user has entered is an one-table-select-statement
        // for example, select * from test.table where \$CONDITIONS
        Pattern ptn = Pattern.compile(pattern);
        Matcher mch = ptn.matcher(query);
        LOG.debug("Using pattern: " + pattern + " to locate table's name.");
        if (mch.find()) {
            LOG.debug("query : " + query + " matches pattern.");
            LOG.debug("found group 1 : " + mch.group(1));
            LOG.debug("found group 2 : " + mch.group(2));
            LOG.debug("found group 3 : " + mch.group(3));
            LOG.debug("found group 4 : " + mch.group(4));
            String tableName = mch.group(4);
            if (tableName.indexOf(".") != -1) {
                tableName = tableName.split("\\.")[1];
            }
            return connManager.getColumnComments(tableName);
        } else {
            LOG.debug("query :" + query + " does not match the pattern.");
        }
    }
      return null;
  }
  ...
  /**
   * Return the column's comment for an specified column name
   * @param columnNames list of map<column name, column comment>
   * @param columnName
   * @return
   */
  public String getOneColumnComment(String columnName, List<Map<String, String>> columnNames) {
    if (columnNames != null) {
  ...
  public String getCreateTableStmt() throws IOException {
    ...
    List<Map<String, String>> colComments = getColumnComments();
    for (Map map : colComments) {
    ...
    for (String col : colNames) {
      ...
      if (options.getFileLayout() == SqoopOptions.FileLayout.TextFile) {
        ...
        // columnNameToAvroFieldSchema: c_date=["null", "string"]
      } else {
      ...
      sb.append(" comment '").append(getOneColumnComment(col, colComments)).append("'");
